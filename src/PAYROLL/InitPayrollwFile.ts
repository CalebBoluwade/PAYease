    ...
    if (req.files) {
      ...
      newFile.mv(path.join(uploadPath), (err: any) => {
        if (err) {
          ...
          paymentSchedule.hasOwnProperty("accountNumber")

          const Fees = Trans_fee * paymentSchedule.length;

          let nameEnquiry: Array<object>;
          let requestParams = { "accountNumber": "0123456789", "requestId": process.env.REQUEST_ID }

          paymentSchedule.forEach((payment: any) => {
            logger.info("Performing beneficiary Account name enquiry for " + payment.Account);

            // Do Name enquiry
            axios.post(process.env.BAL_ENQUIRY_ACCOUNT as string, {
              headers: {
          ...
          let newPath: string = path.join(
            __dirname,
            "Uploads",
            `/PAYMENT_SCHEDULE_${sessionID}.xlsx`
          );
